...
function writeAnagramsFile(nbWords, filename, callback) {
...
function randomWords(options) {
  ...
  if (typeof options.formatter !== 'function') {
    options.formatter = (word: string): string => word;
  }
...
function processor(
  job: { data: { nbWord: number }; id: number },
  cb: (err: Error, result: any) => void,
) {
  const { nbWord } = job.data;
  const filename = `./anagrams-${job.id}.txt`;
  console.log(`Processing ${filename}`);
  writeAnagramsFile(nbWord, filename, () => cb(null, filename));
}
